...
static int	init_philosopher(t_philo *philo, int id, t_time time,
	t_table *table)
{
	philo->fork_left = id;
...
static int	init_table_bis(t_table *table, t_philo *philo,
...
int	init_table(t_table *table, t_philo *philo, t_option option)
{
	...
	return (init_table_bis(table, philo, option, time));
}
